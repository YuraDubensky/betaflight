...
FAST_CODE_NOINLINE void rxFrameCheck(timeUs_t currentTimeUs, timeDelta_t currentDeltaTimeUs)
{
    ...
    if (signalReceived) {
        ...
        isFailsafeMode = false; // turn off flag that is used to mimic rx input when it is lost but MSP is active
    ...
    if (IS_RC_MODE_ACTIVE(BOXMSPOVERRIDE) && rxConfig()->msp_override_channels_mask && rxConfig()->msp_override_failsafe) {
        if (rxMspOverrideFrameStatus() & RX_FRAME_COMPLETE) {
            ...
            rxDataProcessingRequired = true;
            needRxSignalBefore = currentTimeUs + needRxSignalMaxDelayUs;
            isFailsafeMode = true; // set failsafe detection flag to indicate that now values should be taken from MSP if it is active
        }
    }
#endif
...
void detectAndApplySignalLossBehaviour(void)
{
    ...
    for (int channel = 0; channel < rxChannelCount; channel++) {
        // in case Failsafe stage 1 was activated - need to make sure to grab aux values from failsafe and rc from MSP
